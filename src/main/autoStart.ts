...
function makeAutoStartLinuxDesktop(): AutoStart {
    ...
    const file = join(dir, "veskdora.desktop");
    const legacyFile = join(dir, "vesktop.desktop");

    return {
        isEnabled: () => existsSync(file) || existsSync(legacyFile),
        enable() {
            const desktopFile = stripIndent`
                [Desktop Entry]
                Type=Application
                Name=Veskdora
                Comment=Veskdora autostart script
